...
const DEMO_COMISION_ID = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    ...
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 text-red-900 p-4">
        ...
        <p className="text-xs mt-2 text-red-700">
          Asegurate de que classifier-service esté corriendo en el puerto 8008
          y que haya clasificaciones persistidas para la comisión demo.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Clasificaciones N4</h1>
...
function DistributionCard({
  ...
  color: "red" | "yellow" | "green"; emoji: string
}): ReactNode {
  ...
  const textColor = {
    red: "text-red-900", yellow: "text-yellow-900", green: "text-green-900",
  }[color]
  ...
  return (
    <div className={`rounded-lg border p-4 ${bg} ${textColor}`}>
      ...
      <div className="mt-2 flex items-baseline gap-2">
        ...
        <span className="text-sm opacity-70">de {total} ({pct}%)</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex items-end gap-1 h-48">
        {data.map((d) => {
          const total = d.counts.delegacion_pasiva
            + d.counts.apropiacion_superficial
            + d.counts.apropiacion_reflexiva
