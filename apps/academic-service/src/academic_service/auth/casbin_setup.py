...
def check_permission(user: User, resource: str, action: str) -> bool:
    ...
    # Rol 'superadmin' bypasa (excepto para operaciones hard-coded prohibidas)
    if "superadmin" in user.roles:
        return True

    enforcer = get_enforcer()
    # En Casbin el "sub" es el rol prefijado con "role:" en nuestras policies,
    # y los roles del user se resuelven por el g = (user, role, domain) implícito.
    for role in user.roles:
        if enforcer.enforce(
            f"role:{role}", str(user.tenant_id), resource, action
        ):
            return True
    return False
