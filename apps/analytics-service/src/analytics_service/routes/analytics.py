...
async def compute_kappa(req: KappaRequest) -> KappaResponse:
...
async def export_cohort(req: CohortExportRequest) -> ExportJobResponse:
    ...
    # TODO F8: obtener tenant_id y user_id del JWT una vez integremos auth
    # en el analytics-service. Por ahora usamos valores placeholder para el stub.
...
async def get_cohort_progression(comision_id: UUID) -> CohortProgressionOut:
    ...
    # TODO F9: obtener tenant_id del JWT (requiere auth en analytics-service)
    tenant_id = UUID("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")
    ...
    if _real_data_source_enabled():
        ...
        ctr_engine = create_async_engine(os.environ["CTR_STORE_URL"], pool_size=2)
        cls_engine = create_async_engine(os.environ["CLASSIFIER_DB_URL"], pool_size=2)
...
async def ab_test_profiles(req: ABTestRequest) -> ABTestResponse:
