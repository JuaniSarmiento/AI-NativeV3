...
def _real_data_source_enabled() -> bool:
    """True si las env vars de DB real están seteadas."""
    return bool(
        os.environ.get("CTR_STORE_URL") and os.environ.get("CLASSIFIER_DB_URL")
    )
...
def _get_classifier_engine():
    from sqlalchemy.ext.asyncio import create_async_engine
    url = os.environ["CLASSIFIER_DB_URL"]
    return create_async_engine(
        url,
        pool_size=5,
        max_overflow=10,
