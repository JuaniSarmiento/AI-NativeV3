...
def test_default_prompt_version_es_v101_post_g12_activation() -> None:
    """ADR-009 + G12 (2026-04-29): la version activa del prompt tutor es v1.0.1.

    El manifest global (`ai-native-prompts/manifest.yaml`) declara `default.tutor: v1.0.1`.
    Esta config debe coincidir.
    """
    s = Settings()
    assert s.default_prompt_version == "v1.0.1", (
        f"tutor-service.default_prompt_version='{s.default_prompt_version}' "
        f"pero el manifest global del repo apunta a v1.0.1. ALINEAR los dos "
...
def test_manifest_yaml_existe_y_se_parsea() -> None:
    ...
    assert manifest.exists(), (
        ...
        f"hardcodeado mientras el tutor usa v1.0.1 — desalineacion silenciosa."
    )
    ...
    assert "tutor: v1.0.1" in text, (
        f"manifest.yaml debe declarar 'tutor: v1.0.1' bajo active.default; "
