...
export function KappaRatingView({ getToken, episodes }: Props) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-semibold">Inter-rater agreement (Kappa)</h1>
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Compará tu juicio con el del clasificador automático. Clasificá cada
          episodio usando los tres botones; al final obtenés el Kappa de acuerdo.
        </p>
      </div>
          ...
          <div className="space-y-3">
            {episodes.map((ep) => (
              <EpisodeRatingCard
                key={ep.episode_id}
                episode={ep}
                currentLabel={humanLabels[ep.episode_id]}
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <div className="text-xs text-right shrink-0">
          ...
          <div className="font-medium">
            {CATEGORY_LABELS[episode.classifier_label]}
          </div>
        </div>
      </div>
