...
export function ExportView({ getToken, comisionIdDefault = "" }: Props) {
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Exportar dataset académico</h1>
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Genera un dataset anonymizado con los episodios, eventos, y
          clasificaciones N4 de una cohorte. Los student_pseudonym se hashean
          con tu salt de investigación (mismo salt → mismos aliases, reproducible).
        </p>
      </div>
      ...
      {!job && (
        <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1">Comisión (UUID)</label>
            <input
              type="text"
              value={comisionId}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Guardalo en un lugar seguro — sin el salt no podés correlacionar
              datasets posteriores con éste.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Alias de cohorte</label>
              <input
                type="text"
                value={cohortAlias}
      ...
      {error && (
        <div className="p-3 rounded bg-red-50 text-red-900 text-sm">{error}</div>
      )}
    </div>
  )
}
...
function JobProgressPanel({
  ...
  const statusConfig: Record<string, { color: string; label: string; progress: number }> = {
    pending: { color: "bg-slate-400", label: "En cola", progress: 15 },
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 space-y-4">
      <div className="flex items-baseline justify-between">
        ...
        <span className={`px-2 py-0.5 rounded text-xs text-white ${cfg.color}`}>
          {cfg.label}
        </span>
      </div>
      ...
      {job.status === "failed" && job.error && (
        <div className="p-3 rounded bg-red-50 text-red-900 text-sm font-mono">
          {job.error}
        </div>
      )}
