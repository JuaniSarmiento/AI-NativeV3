...
async def check_database(name: str, dsn: str) -> list[str]:
    ...
    for t in tables_without_rls:
        issues.append(f"  {name}.{t} — sin política RLS (usar apply_tenant_rls)")
...
async def main() -> int:
    ...
    if all_issues:
        print("✗ Tablas con tenant_id sin política RLS:")
        for issue in all_issues:
            print(issue)
        return 1

    print("✓ Todas las tablas con tenant_id tienen policy RLS + FORCE")
    return 0
