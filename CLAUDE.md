...
- **Cada episodio cerrado emite attestation externa Ed25519** (ADR-021, RN-128): el ctr-service hace XADD a stream Redis `attestation.requests` post-commit; el `integrity-attestation-service` (puerto 8012, infra institucional separada en piloto) firma y appendea a `attestations-YYYY-MM-DD.jsonl`. **Eventualmente consistente** con SLO 24h — su ausencia **NO bloquea** el cierre del episodio. Buffer canónico bit-exact + formato del `ts` (sufijo `Z`, no `+00:00`) están en RN-128; cualquier desviación ROMPE la verificación. El payload de cierre (`episodio_cerrado` o `episodio_abandonado`, snake_case) no cambia — la attestation es side-channel. **Verificado 2026-05-07**: el stream `attestation.requests` se está disparando (`XLEN = 18` con 17 episodios cerrados al momento de la verificación). El caveat anterior de 2026-05-04 quedó obsoleto. El consumer institucional (`integrity-attestation-service:8012` en VPS UNSL) procesa estos eventos eventualmente; en dev local el stream se acumula sin consumer.
...
2. **Casbin policies completas**: 108 → **131** (verificado 2026-05-07 con `SELECT count(*) FROM casbin_rules WHERE ptype='p'` en `academic_main`). El seed evolucionó: 8 nuevas por `byok_key:CRUD × superadmin/docente_admin` post-ADR-039 + bumps adicionales por extensiones del catálogo de recursos (entregas/calificaciones del epic `tp-entregas-correccion`). RN-018, HU-016, F1-STATE.md ya no especifican un número fijo.
...
- Attestation Ed25519: verificado 2026-05-07 con `XLEN attestation.requests = 18` y 17 episodios cerrados — el path documentado SÍ se está disparando en piloto local. El warning previo (2026-05-04) quedó obsoleto.
...
- **`evaluation-service` IMPLEMENTADO** (epic `tp-entregas-correccion` cerrado): 8 endpoints REST (`/api/v1/entregas` CRUD + `/{id}/submit|calificar|return` lifecycle + `/{id}/calificacion`), 2 modelos (`Entrega`, `Calificacion`), audit log structlog. Comparte DB `academic_main` con academic-service vía engine independiente. **Decisión pendiente para post-v1.0**: candidato natural a fusión con academic-service (Fase 2 del plan de restructure).
...
- **Casbin policies — sin spec hardcodeado**: el source of truth es el código del seed (`apps/academic-service/src/academic_service/seeds/casbin_policies.py`). Hoy carga **131 policies** (verificado 2026-05-07 contra DB; 4 roles × N entidades crecientes; los últimos bumps: +14 por `tarea_practica_template:CRUD` en ADR-016, +1 por `facultad:read` para docente, +8 por `byok_key:CRUD` para superadmin/docente_admin en ADR-039, +más por entregas/calificaciones del epic `tp-entregas-correccion`). RN-018, HU-016, F1-STATE.md ya no especifican un número fijo — evoluciona con el catálogo de recursos.
