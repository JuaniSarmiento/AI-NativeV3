    ...
    async def interact(self, episode_id: UUID, user_message: str) -> AsyncIterator[dict]:
        ...
        if self.overuse_detector is not None:
            ...
            try:
                ...
                overuse_match = await self.overuse_detector.check(
                    state.episode_id, now_ts
                )
            except Exception:
            ...
            if overuse_match is not None:
                ...
                try:
                    await self.ctr.publish_event(
                        ovu_event, state.tenant_id, TUTOR_SERVICE_USER_ID
                    )
                except Exception:
                    logger.warning(
        ...
        if settings.socratic_compliance_enabled:
            ...
            except Exception:  # noqa: BLE001 — fail-soft per ADR-044
