...
Orquesta cada interacción socrática estudiante-tutor: carga el prompt vigente, hace retrieval al material de cátedra, invoca al LLM con streaming SSE, y emite la secuencia de eventos del CTR (`episodio_abierto`, `prompt_enviado`, `tutor_respondio`, `codigo_ejecutado`, `edicion_codigo`, `anotacion_creada`, `episodio_cerrado`).
...
- **NO re-valida JWT**: confía en los headers `X-User-Id`, `X-Tenant-Id`, `X-Role` inyectados por [api-gateway](./api-gateway.md). Tiene su propio `auth/dependencies.py` que sólo los lee.
...
| `POST` | `/api/v1/episodes/{id}/events/edicion_codigo` | Emite snapshot del editor + `diff_chars` + `language`. 202 con `seq`. Crítico para CCD. | Mismos roles. |
| `POST` | `/api/v1/episodes/{id}/events/anotacion_creada` | Emite `NotaPersonal` (reflexión explícita). Alimenta CCD orphan ratio. 202 con `seq`. | Mismos roles. |
...
El estado histórico vive en `ctr_store` (propiedad de [ctr-service](./ctr-service.md)): si la sesión expira por TTL antes de un `episodio_cerrado`, el episodio queda como "abandonado" sin evento de cierre explícito — la cadena de hashes permanece válida hasta el último evento persistido. El contrato `EpisodioAbandonado` en `packages/contracts` contempla este caso (con `reason: "timeout"` y `last_activity_seconds_ago`), pero el emitter no está wireado hoy — queda como deuda.
...
- `apps/tutor-service/src/tutor_service/routes/episodes.py` — los 8 endpoints. `_build_episode_state()` es la proyección CTR→UI que usa `GET /episodes/{id}` para recovery.
- `apps/tutor-service/src/tutor_service/config.py` — URLs a los 5 servicios dependientes + nombres de modelos LLM.
...
────────────────────────────────────────────────────────────────────────────
 ...
 6   anotacion_creada     estudiante real     NotaPersonal — reflexión explícita
...
- `/health` es stub — no valida que los 5 servicios dependientes respondan.
...
- `EpisodioAbandonado` (timeout de sesión) no se emite al expirar el TTL — la sesión se borra silenciosamente. El evento existe en el contrato pero no hay worker que lo dispare.
