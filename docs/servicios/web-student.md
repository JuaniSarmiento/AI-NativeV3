...
- Emitir `edicion_codigo` debounced (snapshot del editor + `diff_chars` + `language`) al `POST /api/v1/episodes/{id}/events/edicion_codigo` — crítico para CCD del clasificador (distingue "tipeando/pensando" de "idle").
